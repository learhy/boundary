...
Each BSR session (`*bsrSession`) transitions through four states:

```
initializing ──CreateConnection()──> active
    active ─────RemoveConnection()──> closing (when last conn closes)
    closing ────finalize()──────────> closed
```

State transitions are guarded by `closeMu` to prevent concurrent modification. The `openConns` counter is atomically incremented on connection creation and decremented on finalization — when it reaches 0, the session transitions to `closing` and finalization runs asynchronously.

**State fields on `bsrSession`:**

| Field | Type | Purpose |
|---|---|---|
| `state` | `atomic.Value[bsrSessionState]` | Current state (initialized to `initializing`) |
| `openConns` | `atomic.Int32` | Active connection count |
| `closeMu` | `sync.Mutex` | Guards state transitions to `closing`/`closed` |
| `bsr` | `*bsr.Session` | Non-nil once session is activated |

**Transition: `activating(bsr *bsr.Session)`**
- Called when `CreateConnection` creates the BSR session
- Acquires `closeMu`, checks state is `initializing`, stores session and transitions to `active`
- Idempotent: if already `active`, no-op

**Transition: `startClose() bool`**
- Called when `FinalizeConnection` decrements `openConns` to 0
- Acquires `closeMu`, checks state is `active`, transitions to `closing`
- Returns `true` if transition succeeded; caller should then invoke `finalizeSession` asynchronously
- Returns `false` if already `closing`/`closed` (duplicate call — safe to ignore)

**Transition: `finalize()`**
- Called by `finalizeSession` goroutine after BSR session is closed
- Acquires `closeMu`, transitions to `closed`
- BSR session is already closed at this point; purely updates the state machine

**Thread safety summary:**
- State transitions: guarded by `closeMu`
- `openConns` increments/decrements: atomic
- Session map access: `sessionsMu` (RWMutex)
- Connection map access: `connsMu` (sync.Mutex)
...
1. ✅ `internal/storage/oss_recording_storage.go` — `ossRecordingStorage` local FS implementation
2. ✅ `internal/daemon/worker/worker_init.go` — sets `recordingStorageFactory` and `recorderManagerFactory` via `init()`
3. ✅ Worker config `RecordingStoragePath` already plumbed: `conf.RawConfig.Worker.RecordingStoragePath`

### Phase 2: SshRecordingManager ✅
4. ✅ `internal/daemon/worker/proxy/ssh/session_manager.go` — `SshRecordingManager` with BSRsession state machine
5. ✅ `recorderManagerFactory` set in `worker_init.go` init
6. ⬜ `internal/daemon/worker/ssh_recording_repo.go` — DB write helpers via controller RPC
   - **Deferred to cycle 5.** `FinalizeSession` / `FinalizeConnection` / `FinalizeChannel` are stub no-ops.

### Phase 3: SSH Handler ✅
7. ✅ `internal/daemon/worker/proxy/ssh/` package
8. ✅ `ssh_init.go` — `proxy.RegisterHandler("ssh", ...)` in `init()`
9. ✅ `handler.go` — `handleProxy()` implementation with InterceptingConn + ConnectionRecorder
10. ✅ `recorder.go` — `ConnectionRecorder` wrapping BSR Connection
11. ✅ `pipe.go` — `InterceptingConn` reads SSH wire frames and emits BSR DataChunks
12. ⬜ Auth redaction — not yet implemented. Auth messages currently recorded in full.

### Phase 4: Integration ⬜
13. ⚠️ `SetSessionMeta` wiring: `handleProxy` does not yet extract session metadata from `protocolCtx` and call `SshRecordingManager.SetSessionMeta`. The worker must call this after receiving `AuthorizeConnection` auth data. **This is the critical missing integration step.**
14. ⬜ Verify controller sends correct `ProtocolContext` for SSH recording targets
15. ⬜ Unit tests for `SshRecordingManager` using `fstest.MemFS`

### What's implemented (this cycle)

```
internal/storage/oss_recording_storage.go   # LocalFS RecordingStorage
internal/daemon/worker/worker_init.go       # Factory init (both factories)
internal/daemon/worker/proxy/ssh/ssh_init.go    # Handler registration
internal/daemon/worker/proxy/ssh/handler.go     # Proxy handler + pipe
internal/daemon/worker/proxy/ssh/pipe.go        # InterceptingConn + BSR chunk emit
internal/daemon/worker/proxy/ssh/recorder.go    # ConnectionRecorder (BSR wrapping)
internal/daemon/worker/proxy/ssh/session_manager.go  # SshRecordingManager
```

### What's deferred

- **DB writes** (`ssh_recording_repo.go`): `recording_session`, `recording_connection`, `recording_channel` rows not yet written. TODO in `session_manager.go:FinalizeSession/FinalizeConnection/FinalizeChannel`.
- **Auth redaction**: SSH auth messages recorded in full. Password/key bytes not stripped.
- **SetSessionMeta wiring**: Worker needs a call site to populate real session metadata into BSR session before close.
- **Unit tests**: No test coverage yet for new packages.
